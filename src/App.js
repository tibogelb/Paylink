...
const KEY_SELLERS = "pl_sellers";
const KEY_OFFERS  = "pl_offers";
const KEY_SALES   = "pl_sales";

// ── Palette claire ────────────────────────────────────────────────────────────
const T = {
  bg:       "#F4F6FA",   // fond général
  surface:  "#FFFFFF",   // cartes
  border:   "#E5E9F2",   // bordures
  border2:  "#D0D7E8",   // bordures plus marquées
  nav:      "#FFFFFF",   // barre de nav
  text:     "#1A1D2E",   // texte principal
  sub:      "#6B7280",   // texte secondaire
  muted:    "#9CA3AF",   // texte discret
  input:    "#F8F9FC",   // fond input
};

const CAT_COLORS  = { PRO: "#059669", Joueur: "#EA580C" };
const CAT_COLORS2 = { PRO: "#D1FAE5", Joueur: "#FFEDD5" }; // bg léger
...
function fmt(p)   { return Number(p).toFixed(2).replace(".", ",") + " €"; }
function fmtDate(d) {
  const dt = new Date(d);
...
const DEFAULT_SELLERS = [
  { id:1, name:"Aymeric", active:true },
  { id:2, name:"Gilles",  active:true },
  { id:3, name:"Joakim",  active:true },
];
const DEFAULT_OFFERS = [
  { id:1, label:"TARIF SUPER PRO - 1 Ditch!",  price_ht:9,  category:"PRO",    qr:null },
  { id:2, label:"TARIF SUPER PRO - 3 Ditchs",  price_ht:24, category:"PRO",    qr:null },
  { id:3, label:"TARIF JOUEUR - 1 Ditch!",      price_ht:5,  category:"Joueur", qr:null },
];

// ── Inputs / buttons helpers ──────────────────────────────────────────────────
const inputStyle = {
  width:"100%", boxSizing:"border-box",
  background:T.input, border:`1px solid ${T.border}`,
  borderRadius:10, padding:"11px 13px", color:T.text,
...
// ── Admin Sellers ─────────────────────────────────────────────────────────────
function AdminSellers({ sellers, setSellers }) {
  const [name, setName] = useState("");
  const [confirmDel, setConfirmDel] = useState(null);

  function add() {
    if (!name.trim()) return;
    const updated = [...sellers, { id:uid(), name:name.trim(), active:true }];
    setSellers(updated); save(KEY_SELLERS, updated); setName("");
  }
  function toggle(id) {
    const updated = sellers.map(s=>s.id===id?{...s,active:!s.active}:s);
    setSellers(updated); save(KEY_SELLERS, updated);
  }
  function doDelete(id) {
    const updated = sellers.filter(s=>s.id!==id);
    setSellers(updated); save(KEY_SELLERS, updated); setConfirmDel(null);
  }
  ...
  return (
    <div style={{ padding:"24px 20px", maxWidth:600, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:22 }}>
        ...
        <button onClick={add} disabled={!name.trim()} style={{ ...btnPrimary, opacity:!name.trim()?0.4:1 }}>+ Ajouter</button>
      </div>
        ...
        {sellers.map((s,i)=>{
          ...
          return (
            <div key={s.id} style={{ background:T.surface, borderRadius:12, padding:"14px 18px", border:`1px solid ${T.border}`, display:"flex", alignItems:"center", gap:14, opacity:s.active?1:0.5, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
              ...
              <button onClick={()=>toggle(s.id)} style={{ background:s.active?T.bg:"#D1FAE5", border:`1px solid ${T.border}`, borderRadius:8, padding:"6px 12px", color:s.active?T.sub:"#059669", fontSize:12, cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:600 }}>{s.active?"Désactiver":"Réactiver"}</button>
...
// ── Admin Offers ──────────────────────────────────────────────────────────────
function AdminOffers({ offers, setOffers }) {
  ...
  function saveOffer() {
    ...
    if (isNaN(ht)) return;
    let updated;
    if (editingId) updated = offers.map(o=>o.id===editingId?{...o,...form,price_ht:ht}:o);
    else updated = [...offers, { id:uid(), ...form, price_ht:ht, qr:null }];
    setOffers(updated); save(KEY_OFFERS, updated); setShowForm(false);
  }
  function doDelete(id) {
    const updated = offers.filter(o=>o.id!==id);
    setOffers(updated); save(KEY_OFFERS, updated); setConfirmDel(null);
  }
  function handleQR(id, file) {
    const reader = new FileReader();
    reader.onload = ev => {
      const updated = offers.map(o=>o.id===id?{...o,qr:ev.target.result}:o);
      setOffers(updated); save(KEY_OFFERS, updated);
    };
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ background:T.surface, borderRadius:18, padding:28, width:"100%", maxWidth:440, border:`1px solid ${T.border}`, boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                ...
                <div style={{ flex:1, background:"#D1FAE5", border:"1px solid #6EE7B7", borderRadius:10, padding:"11px 12px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  <span style={{ color:"#065F46", fontSize:15, fontWeight:800, fontFamily:"'Syne',sans-serif" }}>
                    {form.price_ht ? fmt(ttc(parseFloat(String(form.price_ht).replace(",","."))||0)) : "– €"}
                  </span>
            ...
            <div style={{ display:"flex", gap:10 }}>
              ...
              <button onClick={saveOffer} style={{ flex:2, ...btnPrimary }}>Enregistrer</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div key={offer.id} style={{ background:T.surface, borderRadius:12, padding:"13px 16px", border:`1px solid ${T.border}`, display:"flex", gap:12, alignItems:"center", boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ display:"flex", gap:8, marginTop:3 }}>
                  <span style={{ color:T.muted, fontSize:12 }}>{fmt(offer.price_ht)} HT</span>
                  <span style={{ color:c, fontSize:12, fontWeight:700 }}>{fmt(ttc(offer.price_ht))} TTC</span>
...
// ── Admin Sales ───────────────────────────────────────────────────────────────
function AdminSales({ sellers, sales }) {
  ...
  const sellerStats = sellers.map((s,i)=>{
    const ss=confirmed.filter(x=>x.sellerName===s.name);
    return { ...s, color:SELLER_COLORS[i%SELLER_COLORS.length], totalTTC:ss.reduce((a,x)=>a+(x.priceTTC||0),0), totalGames:ss.reduce((a,x)=>a+(x.nbGames||0),0), count:ss.length };
  ...
  return (
    <div style={{ padding:"24px 20px", maxWidth:820, margin:"0 auto" }}>
      <div style={{ marginBottom:20 }}>
        ...
        <p style={{ color:T.sub, fontSize:13, fontFamily:"'Syne',sans-serif" }}>{confirmed.length} vente{confirmed.length>1?"s":""} confirmée{confirmed.length>1?"s":""}</p>
      </div>

      {/* KPIs */}
      <div style={{ display:"flex", gap:10, marginBottom:22, flexWrap:"wrap" }}>
        {[
          { label:"CA CONFIRMÉ",  value:fmt(confirmed.reduce((a,s)=>a+(s.priceTTC||0),0)),  bg:"#D1FAE5", color:"#065F46", border:"#6EE7B7" },
          { label:"JEUX VENDUS",  value:confirmed.reduce((a,s)=>a+(s.nbGames||0),0),         bg:"#EDE9FE", color:"#5B21B6", border:"#C4B5FD" },
              ...
              return (
                <div key={s.id} style={{ background:T.surface, borderRadius:12, padding:"13px 16px", border:`1px solid ${i===0&&s.count>0?s.color+"55":T.border}`, display:"flex", alignItems:"center", gap:14, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                  ...
                  <div style={{ textAlign:"right" }}>
                    <div style={{ color:s.count>0?s.color:T.muted, fontWeight:900, fontFamily:"'Syne',sans-serif", fontSize:18 }}>{fmt(s.totalTTC)}</div>
        ...
        {filtered.map(s=>{
          ...
          return (
            <div key={s.id} style={{ background:T.surface, borderRadius:10, padding:"12px 16px", border:`1px solid ${T.border}`, display:"flex", alignItems:"center", gap:12, boxShadow:"0 1px 3px rgba(0,0,0,0.04)" }}>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, color:T.text, fontSize:13, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{s.offerLabel}</div>
                <div style={{ color:T.sub, fontSize:11, marginTop:2 }}>
                  {s.sellerName} · Confirmé {fmtDate(s.confirmedAt)}
                  {s.clientName && <span style={{ color:T.text }}> · 👤 {s.clientName}</span>}
              ...
              <div style={{ color:"#7C3AED", fontSize:12, fontWeight:600, flexShrink:0 }}>🎮 ×{s.nbGames}</div>
              <div style={{ color:c, fontWeight:800, fontFamily:"'Syne',sans-serif", fontSize:15, flexShrink:0 }}>{fmt(s.priceTTC)}</div>
            </div>
          );
        })}
...
// ── Sales View ────────────────────────────────────────────────────────────────
function SalesView({ offers, seller, sales, setSales }) {
  ...
  function presentOffer(offer) {
    ...
    const newSale = { id:uid(), sellerId:seller.id, sellerName:seller.name, offerId:offer.id, offerLabel:offer.label, category:offer.category, priceTTC:ttc(offer.price_ht), nbGames:ng, status:"presented", presentedAt:new Date().toISOString(), confirmedAt:null };
    const updated = [...sales, newSale];
    setSales(updated); save(KEY_SALES, updated); setSaleId(newSale.id);
  }
  function confirmSale() {
    if (!saleId) return;
    setConfirming(true);
    const updated = sales.map(s=>s.id===saleId?{...s,status:"confirmed",confirmedAt:new Date().toISOString(),clientName:clientName.trim()||null,comment:comment.trim()||null}:s);
    setSales(updated); save(KEY_SALES, updated);
        ...
        <div style={{ textAlign:"center", maxWidth:360, width:"100%" }}>
          ...
          <div style={{ display:"flex", gap:10, justifyContent:"center", marginBottom:16 }}>
            <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:10, padding:"9px 16px" }}>
              ...
              <div style={{ color:T.sub, fontWeight:700, fontFamily:"'Syne',sans-serif" }}>{fmt(selected.price_ht)}</div>
            </div>
            <div style={{ background:scb, border:`2px solid ${sc}66`, borderRadius:10, padding:"9px 22px" }}>
              ...
              <div style={{ color:sc, fontWeight:900, fontSize:22, fontFamily:"'Syne',sans-serif" }}>{fmt(ttc(selected.price_ht))}</div>
            </div>
          ...
          <div style={{ background:T.surface, borderRadius:20, padding:18, display:"inline-block", boxShadow:`0 8px 40px ${sc}33, 0 2px 12px rgba(0,0,0,0.1)`, border:`3px solid ${confirmed?"#059669":sc}`, transition:"border 0.4s", marginBottom:16 }}>
            {selected.qr
              ? <img src={selected.qr} style={{ width:220, height:220, display:"block", borderRadius:8 }} alt="QR Code paiement"/>
          ...
          <button onClick={confirmSale} disabled={confirming||confirmed} style={{
            width:"100%",
            background: confirmed ? "#D1FAE5" : "linear-gradient(135deg,#059669,#047857)",
            border: confirmed ? "1px solid #6EE7B7" : "none",
            ...
            <button key={offer.id} onClick={()=>presentOffer(offer)} style={{ background:T.surface, border:`1.5px solid ${T.border}`, borderRadius:12, padding:"15px 17px", cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:13, outline:"none", width:"100%", transition:"all 0.15s", boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ display:"flex", gap:8, marginTop:2 }}>
                  <span style={{ color:T.muted, fontSize:11 }}>{fmt(offer.price_ht)} HT</span>
                  <span style={{ color:"#7C3AED", fontSize:11 }}>🎮 {ng} jeu{ng>1?"x":""}</span>
                </div>
              </div>
              <div style={{ textAlign:"right", flexShrink:0 }}>
                <div style={{ color:c, fontWeight:900, fontFamily:"'Syne',sans-serif", fontSize:17 }}>{fmt(ttc(offer.price_ht))}</div>
...
export default function App() {
  const [sellers, setSellers] = useState(()=>load(KEY_SELLERS, DEFAULT_SELLERS));
  const [offers,  setOffers]  = useState(()=>load(KEY_OFFERS,  DEFAULT_OFFERS));
  const [sales,   setSales]   = useState(()=>load(KEY_SALES,   []));
  ...
  if (view==="admin") {
    ...
    return (
      <div style={{ minHeight:"100vh", background:T.bg, fontFamily:"'Syne',sans-serif" }}>
        ...
        />
        {adminTab==="offers"  && <AdminOffers  offers={offers}   setOffers={setOffers}/>}
        {adminTab==="sellers" && <AdminSellers sellers={sellers} setSellers={setSellers}/>}
        {adminTab==="sales"   && <AdminSales   sellers={sellers} sales={sales}/>}
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bg, fontFamily:"'Syne',sans-serif" }}>
      ...
      />
      <SalesView offers={offers} seller={seller} sales={sales} setSales={setSales}/>
